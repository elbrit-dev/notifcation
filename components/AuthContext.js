  ...
  const loadAuthFromStorage = () => {
    if (typeof window !== 'undefined') {
      try {
        ...
        const storedKlyRoleId = localStorage.getItem('klyRoleId');
        ...
        if (storedToken && storedUser && storedAuthType === 'erpnext') {
          ...
          console.log('📱 Loaded auth from localStorage:', {
            ...
            klyRoleId: storedKlyRoleId
          });
        }//employeeId and klyRoleId are stored in localStorage and used to identify the user and their role
                    ...
                    try {
                      ...
                      try {
                        ...
                        if (userEmail) {
                          ...
                          if (userPhone) await window.OneSignal.User.addSms(userPhone);
                          if (EmployeeID) await window.OneSignal.User.addTag("EmployeeID", EmployeeID);
                          if (userDisplayName) await window.OneSignal.User.addTag("Name", userDisplayName);
                        }
                      } catch (oneSignalErr) {
                        console.warn('⚠️ OneSignal setup failed:', oneSignalErr);
                      }
                    } catch (oneSignalErr) {
